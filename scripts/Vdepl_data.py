...
class Vdepl:
    def __init__(self, BEGIN_RUN, disk, ring, is_log, is_all, plot_add, alpha_date_key):
        ...
        self.alpha_date_key = alpha_date_key
    ...
    def create_tgraphs(self):
        ...
        syst_err_func_modx = lambda x: 0.0001 #Volts
        stat_errors = {
            "LAYER1":{
        ...
        for k in self.keys:
            ...
            for modx in self.mods_to_plot:
                ...
                self.tg_dict_add[k][modx] = rt.TGraphAsymmErrors(5, self.vdepl_add[k][modx]["Date"], self.vdepl_add[k][modx]["Vdepl"], array('d', [0]), array('d', [0]), errors_add, errors_add)
    ...
    def falphalog_l1_modified(self, x, p0, p1, p2, p3, corr_phi, N_donor, N_benef_anneal_g1, fluence, N_revers_anneal_g1):
        ret = abs(-N_donor.Eval(x) +
            corr_phi*p3*p0*N_benef_anneal_g1.Eval(x) +
            p1*math.log(corr_phi*fluence.Eval(x)+1) +
            corr_phi*p3*p2*N_revers_anneal_g1.Eval(x))
        return ret
    ...
    def fratio_lin(self, x, p0, p1, p2, corr_phi, N_donor, N_benef_anneal_g1, N_constdamage_g1, N_revers_anneal_g1):
        ret = abs(-N_donor.Eval(x) +
                  corr_phi*p0*N_benef_anneal_g1.Eval(x) +
                  corr_phi*p1*N_constdamage_g1.Eval(x) +
                  corr_phi*p2*N_revers_anneal_g1.Eval(x))
        return ret
    
    def fratio_log(self, x, p0, p1, p2, corr_phi, N_donor, N_benef_anneal_g1, N_constdamage_g1, N_revers_anneal_g1):
        ret = abs(-N_donor.Eval(x) +
                  corr_phi*p0*N_benef_anneal_g1.Eval(x) +
                  p1*math.log(corr_phi*N_constdamage_g1.Eval(x)) +
                  corr_phi*p2*N_revers_anneal_g1.Eval(x))
        return ret
    
    def gc_var(self, out_file, layer, dates_sorted, P0, is_log):
        canva1 = rt.TCanvas("fit_vdepl_phi_ratio","fit_vdepl_phi_ratio",800,800)
        canva1.Draw()
        gc_var_arr = array('d', list())
        phi_ratio_init=0.8
        phi_ratio_fin=1.2
        phi_ratio_step = 0.01
        phi_ratio_arr = [phi_ratio_init+i*phi_ratio_step for i in range(0,int((phi_ratio_fin-phi_ratio_init)/phi_ratio_step)+1)]
        phi_ratio_arr = array('d', phi_ratio_arr)
        thickness = 285.
        multiplier = 1.6021766208e-13/(2*11.68*8.854187817)*thickness*thickness
        if not is_log:
            self.ratio = lambda x, pars: multiplier * \
                self.fratio_lin(x[0], pars[0], pars[1], pars[2], pars[3], self.N_donor,
                            self.N_benef_anneal_g1, self.fluence, self.N_revers_anneal_g1)
        else:
            self.ratio = lambda x, pars: multiplier * \
                self.fratio_log(x[0], pars[0], pars[1], pars[2], pars[3], self.N_donor,
                            self.N_benef_anneal_g1, self.fluence, self.N_revers_anneal_g1)
        
        Vfunc_ratio = rt.TF1("VFuncRatio", self.ratio, 1.49076e9, 1.54862e9, 5)
        Vfunc_ratio.FixParameter(0,1.4e-2)
        Vfunc_ratio.SetParameter(1,1)
        Vfunc_ratio.FixParameter(2,6.7e-2)
        Vfunc_ratio.FixParameter(3,phi_ratio_init)

        for phi_r in phi_ratio_arr: 
            Vfunc_ratio.FixParameter(3,phi_r)
            self.tg_dict[layer].Fit("VFuncRatio","QCN","",
                dates_sorted[P0-1]-1,dates_sorted[-1]+1)
            gc_var_arr.append(Vfunc_ratio.GetParameter(1))
        tgr = rt.TGraph(len(phi_ratio_arr), phi_ratio_arr, gc_var_arr)
        tgr.SetMarkerStyle(8)
        rt.gPad.SetGrid()
        tgr.Draw()
        canva1.SaveAs(out_file)
        return [phi_ratio_arr, gc_var_arr]
    
    def solve_lin(self, x, gA, gY, gC, corr_phi, V):
        thickness = 285.
        multiplier = 1.6021766208e-13/(2*11.68*8.854187817)*thickness*thickness
        ret = V/(corr_phi*gA*self.N_benef_anneal_g1.Eval(x) +
                  corr_phi*gC*self.N_constdamage_g1.Eval(x) +
                  corr_phi*gY*self.N_revers_anneal_g1.Eval(x))/multiplier
        return [ret, multiplier*corr_phi*gA*self.N_benef_anneal_g1.Eval(x),
                  multiplier*corr_phi*gC*self.N_constdamage_g1.Eval(x),
                  multiplier*corr_phi*gY*self.N_revers_anneal_g1.Eval(x)]
    
    def solve_log(self, x, gA, gY, gC, corr_phi, V):
        thickness = 285.
        multiplier = 1.6021766208e-13/(2*11.68*8.854187817)*thickness*thickness
        ret = (V-multiplier*gC*math.log(corr_phi*self.N_constdamage_g1.Eval(x)+1))/(corr_phi*gA*self.N_benef_anneal_g1.Eval(x) +
                  corr_phi*gY*self.N_revers_anneal_g1.Eval(x))/multiplier
        return [ret, multiplier*gC*math.log(corr_phi*self.N_constdamage_g1.Eval(x)+1), multiplier*corr_phi*gA*self.N_benef_anneal_g1.Eval(x), multiplier*corr_phi*gY*self.N_revers_anneal_g1.Eval(x)]
    ...
    def fit_alpha_mods(self, modx, layer, dates_sorted, is_log, fileName, gC, P0,corr_phi):
        ...
        self.tg_dict_add[layer][modx].Fit("VFuncAlpha","QCN","",
                dates_sorted[P0-1]-1,dates_sorted[-1]+1)
    ...
    def fit_gConstants(self):
        ...
        self.falpha_log = lambda x, pars: multiplier * \
            self.falphalog_l1_modified(x[0], pars[0], pars[1], pars[2], pars[3], pars[4], self.N_donor,
                        self.N_benef_anneal_g1, self.fluence, self.N_revers_anneal_g1)
        ...
        my_phigC_arr_lin=self.gc_var("gc_variation_vs_phi_ratio_lin.pdf", k, dates_sorted, P0, False)
        my_phigC_arr_log=self.gc_var("gc_variation_vs_phi_ratio_log.pdf", k, dates_sorted, P0, True )
        
        print "PHI_ARR = %s"%my_phigC_arr_lin[0]
        print "gC_ARR  = %s"%my_phigC_arr_lin[1]
        ...
        # dir_out_dict = {
        #     "apr19": "r_vs_z_2linesfit_apr19",
        #     "jul30": "r_vs_z_2linesfit_jul30",
        # }
        # dir_out = dir_out_dict[self.alpha_date_key]#"r_vs_z_2linesfit_jul30_mod_log"
        dir_out = "hv_scans_fits"
        of_alpha = open("%s/output.txt"%dir_out, "w+")
        
        ldrs = {"mod1": 5, "mod2": 5, "mod4": 6}
        z_arr = list()
        z_arr_err = list()
        ...
        z_arr_dict = {
                    ...
                    "mod4":23.45}
        mod_arr_ = ["mod2", "mod4"]
        for i,modx in enumerate(mod_arr_):
            z_arr.append(abs(fl_pos_dict["BPix_BmO_SEC7_LYR1_LDR%sF_MOD%s"%(ldrs[modx],modx[-1])].z))
            # z_arr.append(z_arr_dict[modx])
            ...
            corr_phi = fl_pos_dict["BPix_BmO_SEC7_LYR1_LDR%sF_MOD%s"%(ldrs[modx],modx[-1])].fluence/aver_phi
            # corr_phi = fl_aver_mods[modx]/aver_phi
            ...
            position_modx = fl_pos_dict["BPix_BmO_SEC7_LYR1_LDR%sF_MOD%s"%(ldrs[modx],modx[-1])]
            # of_alpha.write("%s (%s, %s, %s)\n"%(modx, 2.93, z_arr_dict[modx], fl_aver_mods[modx]))
            of_alpha.write("%s (%4.2f, %s, %s)\n"%(modx, position_modx.r, z_arr_dict[modx], fl_aver_mods[modx]))
            of_alpha.write("real\n")
            of_alpha.write("log: ")
            alpha_solve_log = self.solve_log(self.vdepl_add[k][modx]["Date"][0], self.Vfunc_log.GetParameter(0), self.Vfunc_log.GetParameter(2), self.Vfunc_log.GetParameter(1), corr_phi, self.vdepl_add[k][modx]["Vdepl"][0])
            alpha_solve_lin = self.solve_lin(self.vdepl_add[k][modx]["Date"][0], self.Vfunc.GetParameter(0), self.Vfunc.GetParameter(2), self.Vfunc.GetParameter(1), corr_phi, self.vdepl_add[k][modx]["Vdepl"][0])
            of_alpha.write("alpha = %s \n (%s = (%s - %s)/(%s + %s))\n (%s) \n"%(alpha, alpha_solve_log[0], self.vdepl_add[k][modx]["Vdepl"][0], alpha_solve_log[1], alpha_solve_log[2], alpha_solve_log[3], multiplier*self.N_donor.Eval(self.vdepl_add[k][modx]["Date"][0])))
            corr_phi = 1.
            alpha = self.fit_alpha_mods(modx,k,dates_sorted,is_log = True, fileName = "Vdepl_alpha/%s_alpha_log.pdf"%modx, gC = self.Vfunc_log.GetParameter(1), P0=P0, corr_phi=corr_phi)
            alpha_aver_log = alpha

            alpha = self.fit_alpha_mods(modx,k,dates_sorted,is_log = not True, fileName="Vdepl_alpha/%s_alpha_lin.pdf"%modx, gC = self.Vfunc.GetParameter(1), P0=P0, corr_phi=corr_phi_def)
            ...
            of_alpha.write("lin: ")
            of_alpha.write("alpha = %s\n (%s = %s/(%s + %s + %s)) \n (%s) \n"%(alpha, alpha_solve_log[0], self.vdepl_add[k][modx]["Vdepl"][0], alpha_solve_lin[1], alpha_solve_lin[2], alpha_solve_lin[3], multiplier*self.N_donor.Eval(self.vdepl_add[k][modx]["Date"][0])))
    ...
    def save_vdepl(self,outputFile_name="Vdepl.root"):
        ...
        if self.isfit: self.fit_gConstants()
        self.canv1.Draw()
        ...
        # if self.plot_add:
        #     xmin_ = min(self.vdepl_add[k]['mod1']["Date"]) - 3600*75
        #     xmax_ = max(self.vdepl_add[k]['mod1']["Date"]) + 3600*75
        if self.plot_add:
        ...
        ymin_=100
        ymax_=500
        h0.GetYaxis().SetRangeUser(ymin_,ymax_)
        ...
        # self.tg_dict_add[k]["mod1"].SetMarkerStyle(4)
        ...
        self.tg_dict_add[k]["mod2"].SetMarkerColor(rt.kBlack)
        self.tg_dict_add[k]["mod2"].SetLineColor(rt.kBlack)
        ...
        # self.tg_dict_add[k]["mod3"].SetMarkerStyle(4)
        # self.tg_dict_add[k]["mod3"].SetMarkerSize(1.7)
        # self.tg_dict_add[k]["mod3"].SetMarkerColor(rt.kBlue)
        # self.tg_dict_add[k]["mod3"].SetLineColor(rt.kBlue)
        ...
        self.tg_dict_add[k]["mod4"].SetMarkerStyle(4)
        self.tg_dict_add[k]["mod4"].SetMarkerSize(1.7)
        self.tg_dict_add[k]["mod4"].SetMarkerColor(rt.kMagenta)
        self.tg_dict_add[k]["mod4"].SetLineColor(rt.kMagenta)
...
def main(options, args):
    ...
    myVdepl = Vdepl(BEGIN_RUN, options.disk, options.ring, is_log, is_all, plot_add, "def")
    myVdepl.vdepl_data_init()
    # myVdepl.vdepl_add_data_init(["mod1","mod2","mod3","mod4"])
    ...
    # vdepl_vals = [397.79, 351.16, 308.86, 306.21]
    # vdepl_vals = [260.89, 249.69, 226.32, 210.66]

    vdepl_vals = {
        "apr19": [250.64, 234.54, 212.76, 199.28],
        "jul30": [428.31, 350.85, 295.35, 294.96],
    }
    vdepl_date = {
        "apr19": "2018-04-19 12:00:00",
        "jul30": "2018-07-30 12:00:00",
    } 
    # vdepl_vals = [250.64, 234.54, 212.76, 199.28]
    # vdepl_vals2 = [428.31, 350.85, 295.35, 294.96]
    date_vdepl_dict_add = { 
        "mod1":{
            ...
            "2018-08-17 12:00:00":	[374.69, 0],
            "2018-09-01 12:00:00":	[457.31, 0],
            "2018-09-07 12:00:00":	[467.54, 0],
            "2018-09-26 12:00:00":	[459.89, 0],
            "2018-10-20 12:00:00":	[456.35, 0],
        },
        "mod2":{
            "2018-08-17 12:00:00":	[374.69, 0],
            "2018-09-01 12:00:00":	[457.31, 0],
        ...
        "mod4":{
            ...
            "2018-08-17 12:00:00":	[335.26, 0],
            "2018-09-01 12:00:00":	[361.79, 0],
            "2018-09-07 12:00:00":	[401.65, 0],
            "2018-09-26 12:00:00":	[350.70, 0],
            "2018-10-20 12:00:00":	[349.30, 0],
        },
    } 
    
    # date_vdepl_dict_add = { 
    #     "mod1":{
    #         vdepl_date[input_date]: [vdepl_vals[input_date][0],0.],
            # "2018-04-19 12:00:00": [vdepl_vals[0], 0],
            # "2018-07-30 12:00:00": [vdepl_vals2[0], 0],
            # "2018-08-17 12:00:00":	[450, 0],
            # "2018-09-01 12:00:00":	[500, 0],
            # "2018-09-07 12:00:00":	[550, 0],
            # "2018-09-26 12:00:00":	[500, 0],
            # "2018-10-20 12:00:00":	[550, 0],
            ...
            # "2018-08-17 12:00:00":	[428.7565949010766, 0.],
            # "2018-09-01 12:00:00":	[465.3668049239582, 0.],
            # "2018-09-07 12:00:00":	[465.6845161989947, 0.],
            # "2018-09-26 12:00:00":	[434.5431302303973, 0.],
            # "2018-10-20 12:00:00":	[464.23609443341456, 0.],

            # "2018-08-17 12:00:00": [387.131024862, 0],
            # "2018-09-01 12:00:00": [436.960107639, 0],
            # "2018-09-07 12:00:00": [490.040744543, 0],
            # "2018-09-26 12:00:00": [418.272014838, 0],
            # "2018-10-20 12:00:00": [500.284161987, 0],
        # },
        # "mod2":{
        #     vdepl_date[input_date]: [vdepl_vals[input_date][1],0.],
            # "2018-04-19 12:00:00": [vdepl_vals[1], 0],
            # "2018-07-30 12:00:00": [vdepl_vals2[1], 0],
            # "2018-08-17 12:00:00":	[420, 0],
            # "2018-09-01 12:00:00":	[420, 0],
            # "2018-09-07 12:00:00":	[450, 0],
            # "2018-09-26 12:00:00":	[425, 0],
            # "2018-10-20 12:00:00":	[450, 0],
            ...
            # "2018-08-17 12:00:00":	[397.0110360615524, 0.],
            # "2018-09-01 12:00:00":	[405.07077064567625, 0.],
            # "2018-09-07 12:00:00":	[474.36834276085654, 0.],
            # "2018-09-26 12:00:00":	[424.04272663976116, 0.],
            # "2018-10-20 12:00:00":	[456.8387218422341, 0.],
            
            # "2018-08-17 12:00:00": [372.263514509, 0],
            # "2018-09-01 12:00:00": [398.333189741, 0],
            # "2018-09-07 12:00:00": [468.839669769, 0],
            # "2018-09-26 12:00:00": [404.042877284, 0],
            # "2018-10-20 12:00:00": [485.747671239, 0],
        # },
        # "mod3":{
        #     vdepl_date[input_date]: [vdepl_vals[input_date][2],0.],
            # "2018-04-19 12:00:00": [vdepl_vals[2], 0],
            # "2018-07-30 12:00:00": [vdepl_vals2[2], 0],
        # },
        # "mod4":{
        #     vdepl_date[input_date]: [vdepl_vals[input_date][3],0.],
            # "2018-04-19 12:00:00": [vdepl_vals[3], 0],
            # "2018-07-30 12:00:00": [vdepl_vals2[3], 0],
            # "2018-08-17 12:00:00":	[375, 0],
            # "2018-09-01 12:00:00":	[400, 0],
            # "2018-09-07 12:00:00":	[400, 0],
            # "2018-09-26 12:00:00":	[350, 0],
            # "2018-10-20 12:00:00":	[400, 0],

            # "2018-08-17 12:00:00":	[350, 0],
            # "2018-09-01 12:00:00":	[360, 0],
            # "2018-09-07 12:00:00":	[380, 0],
            # "2018-09-26 12:00:00":	[330, 0],
            # "2018-10-20 12:00:00":	[350, 0],
            ...
            # "2018-08-17 12:00:00":	[357.7322919455149, 0.],
            # "2018-09-01 12:00:00":	[360.2654449710248, 0.],
            # "2018-09-07 12:00:00":	[361.7456835929801, 0.],
            # "2018-09-26 12:00:00":	[338.9979463554233, 0.],
            # "2018-10-20 12:00:00":	[363.0323193240583, 0.],

            # "2018-08-17 12:00:00": [339.682059953, 0],
            # "2018-09-01 12:00:00": [346.67635021,  0],
            # "2018-09-07 12:00:00": [343.586272949, 0],
            # "2018-09-26 12:00:00": [324.280102728, 0],
            # "2018-10-20 12:00:00": [338.934590213, 0],
    #     },
    # } 

    # myVdepl.get_vdepl_additional(date_vdepl_dict_add["mod1"], tag=mykeys[chooseKeys], modx = "mod1")
    myVdepl.get_vdepl_additional(date_vdepl_dict_add["mod2"], tag=mykeys[chooseKeys], modx = "mod2")
    # myVdepl.get_vdepl_additional(date_vdepl_dict_add["mod3"], tag=mykeys[chooseKeys], modx = "mod3")
    myVdepl.get_vdepl_additional(date_vdepl_dict_add["mod4"], tag=mykeys[chooseKeys], modx = "mod4")

    myVdepl.init_vdepl_sim(outputSimFile)
    # myVdepl.get_vdepl_sim()
    myVdepl.init_tgraphs()
    myVdepl.get_Vdepl_terms()
    # myVdepl.fit_gConstants()
    myVdepl.apply_draw_options()
    myVdepl.add_legend(leg_pos, options.disk,options.ring)
    myVdepl.save_vdepl(options.output)
...
parser.add_option("", "--alpha-date", action="store", type="string", dest="alpha_date")
# parser.add_option("", "--alpha-outdir", action="store", type="string", dest="layerTag")
